...
export default function TaxTab({ properties, contractLabor = [], taxYear: propTaxYear }: { properties: Property[]; contractLabor?: { id: string; name: string; paid: number }[]; taxYear?: number }) {
  ...
  const [taxView, setTaxView] = useState<"expenses"|"labor">("expenses");
  const totalContractLabor = contractLabor.reduce((s, c) => s + c.paid, 0);
  const paidContractors = contractLabor.filter(c => c.paid > 0);

  const buildLaborHTML = () => {
    const fmt = (n: number) => new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(n);
    const rows = paidContractors.map(c => `<tr><td style="padding:8px 12px;border-bottom:1px solid #eee">${c.name}</td><td style="padding:8px 12px;border-bottom:1px solid #eee;text-align:right">${fmt(c.paid)}</td><td style="padding:8px 12px;border-bottom:1px solid #eee;text-align:center;font-size:11px;color:#666">${c.paid >= 2000 ? "1099-NEC required" : "No 1099 needed"}</td></tr>`).join("");
    const win = window.open("", "_blank");
    if (win) { win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{font-family:Arial,sans-serif;padding:0.5in;color:#000}table{width:100%;border-collapse:collapse}th{text-align:left;padding:8px 12px;border-bottom:2px solid #000;font-size:12px}@page{margin:0.5in;size:8.5in 11in}</style></head><body>
      <div style="display:flex;justify-content:space-between;margin-bottom:24px">
        <div><h2 style="margin:0">Contract Labor Summary</h2><div style="color:#666;font-size:13px">Tax Year ${year} · Schedule C, Line 11</div></div>
        <div style="text-align:right"><div style="font-size:24px;font-weight:700">${fmt(totalContractLabor)}</div><div style="color:#666;font-size:12px">Total Contract Labor</div></div>
      </div>
      <table><thead><tr><th>Contractor</th><th style="text-align:right">Amount Paid</th><th style="text-align:center">1099 Status</th></tr></thead>
      <tbody>${rows}</tbody>
      <tfoot><tr><td style="padding:10px 12px;font-weight:700;border-top:2px solid #000">TOTAL (Schedule C, Line 11)</td><td style="padding:10px 12px;font-weight:700;text-align:right;border-top:2px solid #000">${fmt(totalContractLabor)}</td><td style="border-top:2px solid #000"></td></tr></tfoot>
      </table>
      <div style="margin-top:20px;font-size:11px;color:#666;line-height:1.5">All contract labor is deductible regardless of 1099 filing requirements. The $2,000 threshold (2026+) only determines whether a 1099-NEC must be filed, not deductibility. Keep payment records as substantiation.</div>
      <div style="margin-top:16px;font-size:10px;color:#999;border-top:1px solid #ccc;padding-top:8px">Generated by GetPaid · Verify with your tax professional before filing.</div>
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ color: C.muted, margin: "6px 0 0", fontSize: 14 }}>For your accountant</p>
        </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {[["expenses", "Deductible Expenses"], ["labor", "Contract Labor"]].map(([id, label]) => (
          <button key={id} onClick={() => setTaxView(id as "expenses"|"labor")}
            style={{ background: taxView === id ? C.accentGlow : "transparent", border: `1px solid ${taxView === id ? C.accent + "44" : C.border}`, borderRadius: 8, padding: "8px 16px", color: taxView === id ? C.accentLight : C.muted, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {taxView === "labor" && (
        <div>
          <div style={{ background: `linear-gradient(135deg, ${C.accent}22, ${C.accentGlow})`, border: `1px solid ${C.accent}44`, borderRadius: 14, padding: "20px 24px", marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: C.accentLight, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>Total Contract Labor {year}</div>
              <div style={{ fontSize: 32, fontWeight: 800, color: C.text, letterSpacing: -1 }}>{$$(totalContractLabor)}</div>
              <div style={{ color: C.muted, fontSize: 13, marginTop: 4 }}>Schedule C, Line 11 · {paidContractors.length} contractor{paidContractors.length !== 1 ? "s" : ""}</div>
            </div>
            <button onClick={buildLaborHTML} disabled={paidContractors.length === 0}
              style={{ background: paidContractors.length === 0 ? C.surface : C.accent, border: "none", borderRadius: 10, padding: "12px 20px", color: paidContractors.length === 0 ? C.muted : "#fff", fontSize: 14, fontWeight: 700, cursor: paidContractors.length === 0 ? "not-allowed" : "pointer" }}>
              📄 Export for Accountant
            </button>
          </div>

          <div style={{ background: C.greenGlow, border: `1px solid ${C.green}44`, borderRadius: 12, padding: "14px 18px", marginBottom: 20, fontSize: 13, color: C.green }}>
            ✓ All contract labor is deductible regardless of 1099 requirements. The $2,000 threshold only affects whether you file a 1099-NEC — not what you can write off.
          </div>

          {paidContractors.length === 0 && (
            <div style={{ textAlign: "center", padding: "50px 0", color: C.muted }}>
              <div style={{ fontSize: 36, marginBottom: 10 }}>👷</div>
              <div style={{ fontWeight: 600, marginBottom: 6 }}>No contract labor paid in {year}</div>
              <div style={{ fontSize: 14 }}>Mark crew payments as paid to track deductible labor</div>
            </div>
          )}

          {paidContractors.sort((a, b) => b.paid - a.paid).map(c => (
            <div key={c.id} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "16px 18px", marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 15 }}>{c.name}</div>
                <div style={{ fontSize: 12, color: c.paid >= 2000 ? C.yellow : C.muted, marginTop: 3 }}>
                  {c.paid >= 2000 ? "⚠️ 1099-NEC required (paid $2,000+)" : "No 1099 needed (under $2,000)"}
                </div>
              </div>
              <div style={{ fontWeight: 800, fontSize: 18, color: C.green }}>{$$(c.paid)}</div>
            </div>
          ))}
        </div>
      )}
