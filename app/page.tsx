...
// ─── Contractor Detail ────────────────────────────────────────────────────────
function ContractorDetail({ contractor, logs, properties, advances, userId, onBack, onTogglePaid, onDeleteLog, onUpdateLog, onUpdateContractor, onDeleteContractor, onAddLog, onAddAdvance, onDeleteAdvance }: {
  contractor: Contractor; logs: Log[]; properties: Property[]; advances: Advance[]; userId: string;
...
export default function GetPaid() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: c }, { data: p }, { data: l }, { data: a }, { data: si }, { data: exps }, { data: ei }, { data: pw }] = await Promise.all([
        ...
        supabase.from("workers").select("contractor_id").not("contractor_id", "is", null),
      ]);
      setPortalLinkedIds(new Set((pw || []).map((w: { contractor_id: string }) => w.contractor_id)));
      setContractors(c || []);
      setProperties(p || []);
      setAdvances(a || []);
      setScopeItems(si || []);
  ...
  const addAdvance = async (advance: Omit<Advance, "id">) => {
    const { data, error } = await supabase.from("advances").insert({ ...advance, user_id: user?.id }).select().single();
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Inter', -apple-system, sans-serif", color: C.text }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
        ...
        {tab === "contractors" && (
          selectedContractor ? (
            <ContractorDetail
              contractor={selectedContractor} logs={logs} properties={properties} advances={advances} userId={user.id}
                    ...
                    <div key={con.id} onClick={() => setSelectedContractor(con)}
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                          ...
                          {portalLinkedIds.has(con.id) && (
                            <span style={{ background: C.greenGlow, color: C.green, border: `1px solid ${C.green}44`, borderRadius: 999, padding: "2px 9px", fontSize: 10, fontWeight: 700, letterSpacing: 0.5, textTransform: "uppercase" as const, whiteSpace: "nowrap" }}>
                              📱 Portal
                            </span>
                          )}
                        </div>
                        {con.job_title ? <div style={{ color: C.accentLight, fontSize: 12, marginTop: 2 }}>{con.job_title}</div> : null}
                        <div style={{ color: C.muted, fontSize: 13, marginTop: 2 }}>{$$(con.rate)}/hr</div>
                      </div>
                      <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
        ...
        {/* INVOICES */}
        {tab === "invoices" && (
          <InvoicesTab properties={properties} userId={user.id} />
        )}

        {/* SCOPE */}
        {tab === "scope" && (
          <ScopeTab properties={properties} logs={logs} contractors={contractors} expenses={[]} advances={advances} userId={user.id} onPropertyUpdate={updatePropertyStatus} />
        )}
        ...
        <Modal title={`Add ${propTermSingular}`} onClose={() => setShowAddP(false)}>
          {usesSites ? (
            ...
            <>
              ...
              <Field label="Job Name" placeholder="e.g. Weekly Lawn 
