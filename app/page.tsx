    ...
    >
      <div
        style={{
          ...
          width: "100%",
          maxWidth: 480,
          boxShadow: `0 24px 80px rgba(0,0,0,0.6), 0 0 0 1px ${COLORS.accent}22`,
        }}
...
function Input({ label, ...props }) {
  return (
    <div style={{ marginBottom: 16 }}>
      {label && (
        <label style={{ display: "block", color: COLORS.textSub, fontSize: 12, fontWeight: 600, marginBottom: 6, letterSpacing: 0.5, textTransform: "uppercase" }}>
          {label}
        </label>
      )}
      <input
        ...
        onFocus={(e) => { e.target.style.borderColor = COLORS.accent; if (props.onFocus) props.onFocus(e); }}
        onBlur={(e) => { e.target.style.borderColor = COLORS.border; if (props.onBlur) props.onBlur(e); }}
      />
    </div>
  );
}
...
function StatCard({ label, value, color, glow, icon }) {
  return (
    ...
    >
      ...
      <div style={{ color: color, fontSize: 24, fontWeight: 800, letterSpacing: -1 }}>{value}</div>
...
export default function GetPaid() {
  ...
  const addContractor = () => {
    ...
    const newC = {
      id: Date.now(),
      name: cForm.name,
      rate: parseFloat(cForm.rate),
      color: CONTRACTOR_COLORS[contractors.length % CONTRACTOR_COLORS.length],
    };
  ...
  const togglePaid = (logId) => {
    setLogs(logs.map((l) => (l.id === logId ? { ...l, paid: !l.paid } : l)));
  };

  const deleteLog = (logId) => setLogs(logs.filter((l) => l.id !== logId));
            ...
            {TABS.map((t) => (
              <button
                ...
                style={{
                  ...
                  border: tab === t.id ? `1px solid ${COLORS.accent}44` : "1px solid transparent",
                  borderRadius: 8,
                  padding: "6px 14px",
                  color: tab === t.id ? COLORS.accentLight : COLORS.textMuted,
                  fontSize: 13,
                  fontWeight: 600,
                  cursor: "pointer",
                  display: "flex",
        ...
        {tab === "dashboard" && (
          <div>
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 32 }}>
              <StatCard label="Total Owed" value={formatCurrency(totalOwed)} color={COLORS.red} glow={COLORS.redGlow} icon="🔴" />
              <StatCard label="Total Paid" value={formatCurrency(totalPaid)} color={COLORS.green} glow={COLORS.greenGlow} icon="✅" />
              <StatCard label="Hours Logged" value={formatHours(totalHours)} color={COLORS.yellow} glow={COLORS.yellowGlow} icon="⏱️" />
              <StatCard label="Contractors" value={contractors.length} color={COLORS.accentLight} glow={COLORS.accentGlow} icon="👷" />
            </div>

            {/* Quick Log Button */}
            <div style={{ marginBottom: 32 }}>
              <Btn onClick={() => setShowLogHours(true)} style={{ fontSize: 15, padding: "12px 28px" }}>
                + Log Hours
              </Btn>
            </div>

            {/* Contractor Breakdown */}
            <h2 style={{ fontSize: 16, fontWeight: 700, marginBottom: 16, color: COLORS.textSub, textTransform: "uppercase", letterSpacing: 1, fontSize: 12 }}>Contractor Breakdown</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 32 }}>
              {contractorSummary.map((c) => (
                ...
                >
                  <div style={{ width: 36, height: 36, borderRadius: 10, background: c.color + "22", border: `1px solid ${c.color}44`, display: "flex", alignItems: "center", justifyContent: "center", color: c.color, fontWeight: 800, fontSize: 14 }}>
                    {c.name.split(" ").map((n) => n[0]).join("")}
                  </div>
                  <div style={{ flex: 1, minWidth: 120 }}>
                    <div style={{ fontWeight: 700, fontSize: 14 }}>{c.name}</div>
                    <div style={{ color: COLORS.textMuted, fontSize: 12 }}>{formatCurrency(c.rate)}/hr · {formatHours(c.hours)} logged</div>
                  </div>
            ...
            {/* Property Breakdown */}
            <h2 style={{ fontSize: 12, fontWeight: 700, marginBottom: 16, color: COLORS.textSub, textTransform: "uppercase", letterSpacing: 1 }}>Property Breakdown</h2>
              ...
              {propertySummary.map((p) => (
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: COLORS.textMuted, fontSize: 12 }}>{p.city} · {p.logCount} entries · {formatHours(p.hours)}</div>
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {contractors.map((c) => {
                ...
                return (
                  ...
                  >
                    <div style={{ width: 48, height: 48, borderRadius: 14, background: c.color + "22", border: `2px solid ${c.color}44`, display: "flex", alignItems: "center", justifyContent: "center", color: c.color, fontWeight: 800, fontSize: 16 }}>
                      {c.name.split(" ").map((n) => n[0]).join("")}
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                      <div style={{ textAlign: "center" }}>
                        <div style={{ color: COLORS.text, fontWeight: 700, fontSize: 15 }}>{formatHours(summary.hours)}</div>
                        <div style={{ color: COLORS.textMuted, fontSize: 11, textTransform: "uppercase", letterSpacing: 0.5 }}>Total Hours</div>
                      </div>
                      <div style={{ textAlign: "center" }}>
                        <div style={{ color: COLORS.red, fontWeight: 700, fontSize: 15 }}>{formatCurrency(summary.owed)}</div>
                        <div style={{ color: COLORS.textMuted, fontSize: 11, textTransform: "uppercase", letterSpacing: 0.5 }}>Owed</div>
                      </div>
                      <div style={{ textAlign: "center" }}>
                        <div style={{ color: COLORS.green, fontWeight: 700, fontSize: 15 }}>{formatCurrency(summary.paid)}</div>
                        <div style={{ color: COLORS.textMuted, fontSize: 11, textTransform: "uppercase", letterSpacing: 0.5 }}>Paid</div>
                      </div>
                    </div>
                    <button
                      onClick={() => setContractors(contractors.filter((x) => x.id !== c.id))}
                      style={{ background: "none", border: "none", color: COLORS.textMuted, cursor: "pointer", fontSize: 16, padding: 4 }}
                    >
                      🗑️
                    </button>
                  </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {properties.map((p) => {
                ...
                const pLogs = logs.filter((l) => l.propertyId === p.id);
                const assignedContractors = [...new Set(pLogs.map((l) => l.contractorId))].map((id) => contractors.find((c) => c.id === id)).filter(Boolean);
                return (
                  <div
                    key={p.id}
                    style={{
                      ...
                      <div style={{ flex: 1, minWidth: 180 }}>
                        ...
                        <div style={{ display: "flex", gap: 6, marginTop: 12, flexWrap: "wrap" }}>
                          {assignedContractors.map((c) => (
                            <span key={c.id} style={{ background: c.color + "22", color: c.color, border: `1px solid ${c.color}44`, borderRadius: 999, padding: "2px 10px", fontSize: 11, fontWeight: 700 }}>
                              {c.name.split(" ")[0]}
                            </span>
                          ))}
                          {assignedContractors.length === 0 && <span style={{ color: COLORS.textMuted, fontSize: 12 }}>No contractors assigned yet</span>}
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                        <div style={{ textAlign: "center" }}>
                          <div style={{ color: COLORS.text, fontWeight: 700, fontSize: 15 }}>{formatHours(summary.hours)}</div>
                          <div style={{ color: COLORS.textMuted, fontSize: 11, textTransform: "uppercase", letterSpacing: 0.5 }}>Hours</div>
                        </div>
                        <div style={{ textAlign: "center" }}>
                          <div style={{ color: summary.owed > 0 ? COLORS.red : COLORS.green, fontWeight: 700, fontSize: 15 }}>{formatCurrency(summary.owed)}</div>
                          <div style={{ color: COLORS.textMuted, fontSize: 11, textTransform: "uppercase", letterSpacing: 0.5 }}>Owed</div>
                        </div>
                      </div>
                      <button
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 28 }}>
              <div>
                ...
                <p style={{ color: COLORS.textMuted, margin: "6px 0 0", fontSize: 14 }}>{logs.length} entries · {logs.filter((l) => !l.paid).length} unpaid</p>
              </div>
              ...
              {[...logs].sort((a, b) => new Date(b.date) - new Date(a.date)).map((log) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: COLORS.card,
                      border: `1px solid ${log.paid ? COLORS.green + "33" : COLORS.border}`,
                      borderRadius: 12,
                      padding: "16px 20px",
                      display: "flex",
                      alignItems: "center",
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap" }}>
                      <div>
                        <div style={{ color: COLORS.text, fontWeight: 700, fontSize: 14 }}>{formatHours(log.hours)}</div>
                        <div style={{ color: COLORS.textMuted, fontSize: 11 }}>{log.date}</div>
                      </div>
                      <div style={{ fontWeight: 700, fontSize: 15, color: log.paid ? COLORS.green : COLORS.yellow }}>{formatCurrency(amount)}</div>
                      <Btn
                        variant={log.paid ? "ghost" : "success"}
      ...
      {showLogHours && (
        <Modal title="Log Hours" onClose={() => setShowLogHours(false)}>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <div style={{ flex: 1 }}>
              <Input label="Hours Worked" type="number" placeholder="8" value={lForm.hours} onChange={(e) => setLForm({ ...lForm, hours: e.target.value })} />
            </div>
            <div style={{ flex: 1 }}>
              <Input label="Date" type="date" value={lForm.date} onChange={(e) => setLForm({ ...lForm, date: e.target.value })} />
            </div>
          </div>
          ...
          {lForm.contractorId && lForm.hours && (
            <div style={{ background: COLORS.accentGlow, border: `1px solid ${COLORS.accent}33`, borderRadius: 8, padding: "10px 14px", marginBottom: 16, fontSize: 13, color: COLORS.accentLight }}>
              💰 This will log <strong>{formatCurrency((contractors.find(c=>c.id==lForm.contractorId)?.rate||0)*parseFloat(lForm.hours||0))}</strong> owed
            </div>
          )}
